import { ExternalGroupData, GroupSyncProvider } from "@server/utils/GroupSyncProvider";
...
class DiscordGroupSyncProvider implements GroupSyncProvider {
    async fetchUserGroups(accessToken: string, settings: AuthenticationProviderSettings): Promise<ExternalGroupData[]> {
        ...
        const currentGuildUser = await userDiscordClient.users.getGuildMember(env.DISCORD_SERVER_ID)

        /** The bot's API Client (used to access role metadata) */
        const discordBotClient = new DiscordAPI(new REST({ authPrefix: "Bot" }).setToken(env.DISCORD_BOT_TOKEN))

        /** The Discord server's role list */
        const guildRoles = await discordBotClient.guilds.getRoles(env.DISCORD_SERVER_ID)

        /** The roles that the user has with server metadata like name (and colour) */
        const overlappingRoles = guildRoles.filter(guildRole => currentGuildUser.roles.includes(guildRole.id))

        /** Outline compatible group objects */
        const externalRoles: ExternalGroupData[] = overlappingRoles.map((overlappingRole) => ({
